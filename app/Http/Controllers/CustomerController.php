...
class CustomerController extends Controller
{
    ...
    public function destroy($id)
    {
        try {
            ...
            if($user->role==0){               
                $Order = StoreOrder::where('customer_id',$id)->get();
                if(count($Order)>0){
                    foreach ($Order as $value) {                                                
                        $OrderChild = StoreOrderChild::where('order_id',$value->id)->get();
            ...
            if($user->role==2){
                ...
                $StoreOrder = StoreOrder::where('deliveryBoy_id',$id)
                ->whereIn('order_status',['PickUpStore','OutOfDelivery'])
                ->get();
            ...
            if($user->role==1){
                ...
                $StoreItem = StoreItem::where('user_id',$id)->get();
                if(count($StoreItem)>0){
                    foreach ($StoreItem as $value) {
                        $value->delete();
                    }
                }
                $StoreSubCategory = StoreSubCategory::where('owner_id',$id)->get();
                if(count($StoreSubCategory)>0){
                    foreach ($StoreSubCategory as $value) {
                        $value->delete();
                    }
                }
                    
                $StoreOrder = StoreOrder::where('owner_id',$id)->get();
                if(count($StoreOrder)>0){
                    foreach ($StoreOrder as $value) {
                        ...
                        $StoreOrderChild = StoreOrderChild::where('order_id',$value->id)->get();
                        if(count($StoreOrderChild)>0){
                            foreach ($StoreOrderChild as $oc) {
                                $oc->delete();
                            }
                        }                   
                        $value->delete();
                ...
                $gShops = StoreShop::where('user_id',$id)->get();
                if(count($gShops)>0){
