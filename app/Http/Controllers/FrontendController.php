...
class FrontendController extends Controller
{
    ...
    public function products(Request $request)
    {
        ...
        $data['products'] = StoreItem::where([['name','like','%'.$query.'%'],['status',0]])->orderBy('id','desc');
    
        if (session()->exists('location_id') && session()->exists('location_name')) {
            $location_id = session('location_id');
            $shop_ids = StoreShop::where('location',$location_id)->get()->pluck('id');
            $data['products'] = $data['products']
                ->whereIn('shop_id',$shop_ids);
        }
        $data['products'] = $data['products']->get();
        
        $default_cat = StoreCategory::where('status',0)->first();
    ...
    public function wishlist()
    {
        $currency_code = Setting::where('id',1)->first()->currency;
        $data['currency'] = Currency::where('code',$currency_code)->first()->symbol;
        $data['wishlist'] = Wishlist::with('storeItem')->where('user_id',Auth::user()->id)->orderBy('id','desc')->get();
    ...
    public function home()
    {
        if(env('DB_DATABASE') == "" || env('DB_DATABASE') == null){
            ...
            $currency_code = Setting::where('id',1)->first()->currency;
            $data['currency'] = Currency::where('code',$currency_code)->first()->symbol;
            $data['category'] = StoreCategory::where('status',0)->orderBy('id','desc')->get();
            $data['banner'] = Banner::where('status',0)->orderBy('id','desc')->get();
            $data['coupon'] = Coupon::where('status',0)->orderBy('id','desc')->get();
            $cat = StoreSubCategory::where('name','like','%fruit%')->orWhere('name','like','%vegetable%')->get()->pluck('id');
            $data['fruits_veg'] = StoreItem::where('status',0)
                ->whereIn('subcategory_id',$cat)
                ->orderBy('id','desc')->get();

            $data['new_product'] = StoreItem::where('status',0)
                ->orderBy('id','desc')->take(10)->get();

            $data['top_featured'] = StoreItem::where([['status',0],['top_featured',1]])
                ->orderBy('id','desc')->take(10)->get();
    ...
    public function allProducts(Request $request)
    {
        ...
        if(isset($default_cat))
        {
            if(isset($request->category_id)){
                $default_cat = StoreCategory::find($request->category_id);
            }
            $data['category'] = StoreCategory::where('status',0)->get();
    
            $data['products'] = StoreItem::where([['status',0],['category_id',$default_cat->id]]);
            if (session()->exists('location_id') && session()->exists('location_name'))
            {
                $location_id = session('location_id');
                $shop_ids = StoreShop::where('location',$location_id)->get()->pluck('id');
    ...
    public function featuredProducts(Request $request)
    {
        ...
        if(isset($default_cat))
        {
            ...
            $data['products'] = StoreItem::where([['status',0],['category_id',$default_cat->id],['top_featured',1]]);
    ...
    public function singleProduct($id, $name)
    {
        ...
        $currency_code = Setting::where('id',1)->first()->currency;
        $data['currency'] = Currency::where('code',$currency_code)->first()->symbol;
        $data['category'] = StoreCategory::find($product->category_id);
        $data['top_featured'] = StoreItem::where([['status',0],['top_featured',1],['id','!=',$id]])
            ->orderBy('id','desc')->take(10)->get();
        $data['like_this'] = StoreItem::where([['status',0],['category_id',$product->category_id],['subcategory_id',$product->subcategory_id],['id','!=',$id]])
            ->orderBy('id','desc')->take(10)->get();
    ...
    public function categotyProduct(Request $request,$id, $name)
    {
        ...
        $data['products'] = StoreItem::where([['status',0],['category_id',$category->id]]);
    ...
    public function dashboard()
    {
        $currency_code = Setting::where('id',1)->first()->currency;
        $data['currency'] = Currency::where('code',$currency_code)->first()->symbol;
        $order = StoreOrder::with('orderItem')->where('customer_id',Auth::user()->id)->orderBy('id','desc')->first();
    ...
    public function profileOrders()
    {
        ...
        $orders = StoreOrder::where('customer_id',Auth::user()->id)->orderBy('id','desc')->get();
    ...
    {
        $used = StoreOrder::where('address_id',$id)->first();
        if(isset($used))
        {
            return false;
        }
    ...
    public function addToCart(Request $request)
    {
        ...
        $product = StoreItem::find($request->item_id);
    ...
    public function getProductDetails(Request $request)
    {
        ...
        $item = StoreItem::find($request->product_id);
    ...
    public function placeOrder(Request $request)
    {
        ...
        $data['order_status'] = Setting::find(1)->default_store_order_status;
        ...
        if($cart)
        {
            ...
            {
                $shop_id = StoreItem::find($item['id'])->shop_id;
                ...
                StoreOrderChild::create($child);
            }
        }
        $order = StoreOrder::with('address')->find($order->id);
        $currency_code = Setting::where('id',1)->first()->currency;
        $data['currency'] = Currency::where('code',$currency_code)->first()->symbol;
    ...
    {
        $order = StoreOrder::with('address')->find($id);
    ...
    {
        $order = StoreOrder::with('address','orderItem')->find($id);
        $currency_code = Setting::where('id',1)->first()->currency;
        $data['currency'] = Currency::where('code',$currency_code)->first()->symbol;
