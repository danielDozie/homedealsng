...
class UserApiController extends Controller
{
    ...
    public function location()
    {
        ...
        foreach ($locations as $key => $location) {
            $shops = StoreShop::where([['status',0],['location',$location->id]])->get();
            foreach ($shops as $key_shop => $shop) {
                $item = StoreItem::where([['status',0],['shop_id',$shop->id]])->first();
    ...
    {
        $category = StoreCategory::where('status',0)->orderBy('id','desc')->get();
    ...
    public function home(Request $request)
    {
        ...
        $data['category'] = StoreCategory::where('status',0)->orderBy('id','desc')->get(['id','name','image']);
    ...
    {
        $data['all'] = StoreOrder::with(['customer','deliveryGuy'])
