                    ...
                    <div class="card shadow">
                        <div class="card-header border-0">
                            <div class="row align-items-center">
                                <div class="col-8">
                                    <h3 class="mb-0">{{ __('Store Orders') }}</h3>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach ($orders as $order)
                                        <tr>
                                            ...
                                            <td class="changeStoreOrderStatus">
                                                <select id="order-{{$order->id}}" name="order_status" class="form-control" {{$order->order_status=="Cancel" || $order->order_status=="Completed" ? 'disabled':''}}>
                                                    ...
                                                    @elseif($order->order_status == 'Approved')
                                                        ...
                                                        <option value="PickUpStore"{{$order->order_status == 'PickUpStore'?'Selected' : ''}}>Pickup Store at Store</option>
                                                        <option value="OutOfDelivery"{{$order->order_status == 'OutOfDelivery'?'Selected' : ''}}>Order is Out Of Delivery</option>
                                                        <option value="Completed"{{$order->order_status == 'Completed'?'Selected' : ''}}>Completed</option>
                                                    @elseif($order->order_status == 'PickUpStore')
                                                        <option value="PickUpStore"{{$order->order_status == 'PickUpStore'?'Selected' : ''}} disabled>Pickup Store at Store</option>
                                                        <option value="OutOfDelivery"{{$order->order_status == 'OutOfDelivery'?'Selected' : ''}}>Order is Out Of Delivery</option>
                                                        <option value="Completed"{{$order->order_status == 'Completed'?'Selected' : ''}}>Completed</option>   
                                            ...
                                            <td class="changeStoreOrderPaymentStatus">
                                            ...
                                            <td>
                                                <a href="{{url('viewStoreOrder/'.$order->id.$order->order_no)}}" class="table-action" data-toggle="tooltip" data-original-title="View Order">
