...
@section('content')
    @include('admin.layout.topHeader', [
        'title' => __('Edit Store Category') ,
        'headerData' => __('Store Category') ,
        'url' => 'StoreCategory' ,
        'class' => 'col-lg-7'
    ])
                        ...
                        <div class="card form-card bg-secondary shadow">
                            <div class="card-header bg-white border-0">
                                <div class="row align-items-center">
                                    <div class="col-8">
                                        <h3 class="mb-0">{{ __('Edit Store Category') }}</h3>
                                    </div>
                                    <div class="col-4 text-right">
                                        <a href="{{ url('StoreCategory') }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                <form method="post" action="{{url('StoreCategory/'.$data->id)}}" autocomplete="off" enctype="multipart/form-data" >
                                    @csrf
                                    @method('put')
                                    <h6 class="heading-small text-muted mb-4">{{ __('Store Category Detail') }}</h6>
                                    <div class="pl-lg-4">
